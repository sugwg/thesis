...
\begin{tikzpicture}
  ...
  \begin{scriptsize}
  \node[circle,draw=black] (sum1) at (-3,-3) {$+$};
  \node[circle,draw=black] (sum2) at (3,0) {$+$};
  \node[analog] (F) at (0,-6) {Active Feedback};
  \node[optical] (O) at (0,-3) {Optical Spring};
  \node[physical] (S) at (0,3) {Small Mass Suspension};
  \node[physical] (CTE) at (0,0) {Thermal Optic Expansion};
  \coordinate (j1) at (-3,-6) {};
  \coordinate (j2) at (-3,-3) {};
  \coordinate (j3) at (-3,0) {};
  \coordinate (j4) at (-3,3) {};
  \coordinate (j5) at (3,-6) {};
  \coordinate (j6) at (3,-3) {};
  \coordinate (j7) at (3,0) {};
  \coordinate (j8) at (3,3) {};
  \draw[thick] (F) -- (j1);
  \draw[thick,->] (O) -- (sum1);
  \draw[thick,->] (j3) -- (CTE);
  \draw[thick,->] (j4) -- (S);
