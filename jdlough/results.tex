...
I will review the import points of the optical spring theory.
...
We can think of it as if we had a device that required the mirror to follow a
sinusoidal motion at a specific amplitude and frequency regardless of external
forces and placed in a cavity to form an optical spring, work would be done on
the optic by the spring and taken up by the special device.
...
This is illustrated in figure \ref{fig:otstabletime}.

Of course, we can also solve the equations of motion,
\begin{align}
F = ma &= -kx \\
  m\ddot x &= -kx\;.
\end{align}
...
\end{align}
With harmonic motion If we allow k to be a complex number, $k = k_0(1+i\phi)$, we have
\begin{align}
x &= Ae^{i\sqrt{k_0e^{i\phi}/m}t} + Be^{-i\sqrt{k_0e^{i\phi}/m}t} \\
  &= Ae^{ie^{i\phi/2}\sqrt{k_0/m}t} + Be^{-ie^{i\phi/2}\sqrt{k_0/m}t} \;.
\end{align}
...
present the results though as they were measured and compare with the
...
\begin{tikzpicture}
  ...
  \addplot[very thick,samples=200,domain=-720:720,blue!80!black] {sin(x)};
  \addlegendentry{$\sin (x)$};
  \addplot[very thick,samples=200,domain=-720:720,red!80!black] {-sin(x+20)};
  \addlegendentry{$-\sin (x+20)$};
  \addplot[very thick,samples=200,domain=-720:720,green!80!black] {-cos(x)*sin(x+20)};
  \addlegendentry{$-\cos(x) \sin (x+20)$};
  \end{axis}
  \begin{axis}[
    name=plot2,
