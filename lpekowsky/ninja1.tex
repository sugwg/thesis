...
We see from
Figure~\ref{fig:ninjapsd} that the noise power spectrum of the NINJA
data set closely approximates the Initial LIGO design sensitivity in
the frequency range of interest ($30$-$10^3\,$~Hz).
...
calculation of the Virgo design curve~\cite{SIESTA}. The $1/f$ tails
...
A list of the data-analysis contributions is shown in Table~\ref{tab:allda}. 
%
\begin{table}
\begin{center}
...
\end{tabular}
\end{center}
\caption[The data-analysis contributions to the NINJA project.]{
\label{tab:allda}
The data-analysis contributions to the NINJA project.}
\end{table}
...
Finally, it is necessary to specify a cutoff frequency at which to
...
We now extend the analysis to include data from the simulated Virgo
...
We investigate three choices of detection statistic: (i) the combined
...
Table~\ref{tab:threshold_found} shows the threshold and the number of triggers
