...
an upcoming paper by Josh Smith and Andrew Lundgren.  Three primary
tools were run either continuously or with low-latencies to provide
views into the data, although these were supplemented with numerous
short special-purpose scripts.  Two of these tools, the klinewelle
...
given threshold.  Both these tools were run on many auxiliary channels
in addition to the detector output.  The third tool, daily ihope, is
the focus of the current chapter.  We proceed as follows:  
...
discussed in section~\ref{sec:applications_dog}.
...
black holes.  During S6/VSR2,3 the goal of the group was to run the
full analysis every two weeks with latencies as low as possible.
...
The structure of the full search is shown in
figure~\ref{f:hipe}~\footnote{Figure by Collin Capano, used with
permission} and discussed in section~\ref{sec:search_pipeline}.  We
will not go over it in detail here, but note that it is a two-stage,
coincident pipeline.
...
displayed results for categories 0, 1, 1+2, and 1+2+4.
...
described in section~\ref{ssec:analysis_trigger_selection}. For each
...
replicated for each 2048-second block and for each IFO.}
\end{figure}%
...
in~\cite{Chatterji:thesis}.  Omega scans are a kind of time-frequency
...
aid to detect coupling between auxiliary channels and \darmerr.  These
can be used to suggest mechanisms behind glitches, especially those
that were not already marked by a DQ flag.  In addition, over time
repeated shapes in the omega scan can pinpoint underlying problems in
the instruments that need to be addressed.  A few images from one of
the glitches in figure~\ref{f:daily_ihope_loudest} is shown in
...
of this event is shown in figure~\ref{f:daily_loudest_glitch}, and it
...
\ref{ssec:analysis_trigger_selection}.  There is a further
...
tended to be individual runs of the \texttt{trigbank} program (see
...
second stage.  Comparing the times that caused problems to the daily
pages immediately revealed a correlation -- times over which
\texttt{trigbank} were unable to run were those where the rates of
triggers were abnormally high.
...
plots of injections in the same data made with the different mass
ranges.  These comparisons showed that the search with the smaller
bank recovered injections in the mass range as well or better than the
larger bank.
...
the mirror overheated and deformed more than
expected~\cite{LIGO-T050064-00-R}.  To correct for this a compensating
laser was added that would heat the mirror in a ring in order to
restore the radius of curvature to an optimal value.  However, this
laser would occasionally glitch, kicking the mirror through radiation
pressure and producing loud noise in the detector.
...
generated by klinewelle and Omega.  The cause was similarly easy to identify,
...
(section~\ref{sec:search_pipeline}).  In the full analysis
...
\emph{SeisVeto}~\cite{Macleod:2011} compared low-frequency triggers
from Omega running on seismic PEM channels to daily ihope triggers
using the \emph{HVeto}~\cite{Smith:2011} algorithm.  Times of high
correlation were flagged with category 4 vetoes, which could achieve
up to 62\% efficiency with only 6\% deadtime.
...
We now move from general uses of daily ihope to its applications in
following up a particular event.

In addition to the hardware injections discussed in
section~\ref{sec:search_pipeline} it was known at the start
of S6 that there would be any from zero to ``a few'' unannounced,
blind hardware injections performed in order to provide an unbiased
test of the search pipelines.  One such injection was performed on Sep
16 2010 at 06:42 UTC, and showed up in multiple searches as a strong
...
results paper, in preparation as of the time of writing.
...
section~\ref{ssec:analysis_trigger_selection}.  The red lines are the
...
\emph{front-end control code}, which is responsible for interfacing
between the data-acquisition and data-recording systems.  This code is
updated occasionally as new systems are added or bugs are found and
fixed.   To verify that the most recent change preceding the event did
not significantly change the behavior of the instruments we compared
histograms of triggers from daily ihope before and after these
changes.
...
We have seen in section~\ref{sec:applications_pipeline} that daily ihope
...
Recall from section~\ref{ssec:analysis_trigger_selection} that
...
addition, denote multiplication of functions by $\cdot$ and convolution
by $\star$.
...
\delta(t)$ then $\tilde{s} = M$  and the SNR time series is then
...
width of the plateaus in figure~\ref{f:impulses_original_no_chisq} (as
...
shown in figure~\ref{f:short_snr_series}.
...
figure~\ref{f:daily_ihope_tcs}.  This effect is confined to the
2048-second analysis segment containing the glitch, as can be seen
from figure~\ref{f:move_glitch}, which shows the trigger rate around a
