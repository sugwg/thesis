...
\weakheader{4. Hardware injections}
...
Livingston detector, characterized by a sudden drop in {\texttt
DARM\_ERR} immediately followed by a sharper increase.  In some cases
there were a few cycles of ringing before the channel settled back
down.  These often produced SNR values of several thousand.  The
$\chisq$ values were typically large for the events themselves,
resulting in negligable $\newsnr$ values.  However, the triggers in
the tails (see section~\ref{sec:penguins}) could have large SNR
values.  A sample daily ihope plot with several spike glitches and 
representative omega scans are shown in figure~\ref{f:daily_spikes}.
...
layer of active isolation \Note{Fill in, cite}.
...
The HEPI glitch in daily ihope and Omega.  On the top left, the
SNR values for the day including a HEPI glitch at 17:38, as identified
by the ``loudest glitches'' report.  On the top right, this event in 
{\texttt DARM\_ERR}.  On the bottom left the time-frequency plot of a
HEPI auxiliary channel, and on the bottom right the unfiltered time
series of this same channel, showing that it has exceeded the
threshold of 25,000}
\end{figure}%

\Note{add SNR histogram plot}

\subsection{Category 4}

% https://wiki.ligo.org/foswiki/bin/view/DetChar/H1LVEASEISZS6Cflag

\weakheader{Use-percentage vetoes}
...
This is a fairly quiet glitch, with an SNR of \Note{SNR}, and it would
have taken considerable effort to track down manually.  However, it is
accompanied by 4 seconds of elevated trigger rates which could have
impacted the search, and is clearly not a gravitational wave, and
should therefore be flagged at category 4.
...
bottom the omega scan of {\texttt DARM\_ERR} and the auxiliary channel 
whose KW triggers correlate with daily ihope {\texttt DARM\_ERR}
triggers.}
\end{figure}%
...
detectors as well as in dual detector coinci- dence. Such an
...
although this will be better accomplished with more time slides.
