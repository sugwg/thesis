In chapter~\ref{ch:detectors} we noted the presence of
...
Ideal, glitches would be removed at the source, by addressing the
...
them from analyses all fall into the category of \emph{detector
characterization}.
...
file.  This file may be thought of as the product of detector
characterization studies.  In broad terms the goal is to identify
glitches, correlate them with existing flags or create new ones, and
remove the glitches from analyses by associating the flag with a veto
severity level through the veto definer file.  One of the most
significant features of the S6 run is that the data analysts working
on detector characterization were in close contact with the people
commissioning the instrument.  This meant that, in addition to
...
scripts.  Two of these tools, the Klinewelle
...
Therefore a number of simplifications were applied to the daily ihope
bank.
...
A version of the veto definer file designated as ``online'' was placed
on a web site from where it could be downloaded by daily ihope.  This
file was used to generate veto segments using the
\verb|ligolw_segments_from_cats| program discussed in
section~\ref{ssec:from_cats}.  Any trigger that fell within the vetoed
segments was removed.

The structure of daily ihope is summarized in
figure~\ref{f:daily_flowchart}.  The result is, for each 2048-second
...
which are run on all auxiliary channels recorded by the instruments
...
in figure~\ref{f:daily_ihope_loudest_omega}.  Finally, note that of
...
compare different veto levels to get a sense of how well the cumulative
...
section~\ref{sec:daily_ihope_open_questions}.  Hence it was decided to
...
Despite a great deal of effort the cause of these glitches was never
...
This was done using the \emph{HVeto}~\cite{Smith:2011} algorithm,
which determines the significance of the correlation between two
channels by computing
%
...
figure~\ref{f:impulses_original_no_chisq}.  The expected behavior is
