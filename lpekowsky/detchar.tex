In chapter~\ref{ch:ligo_detectors} we noted the presence of
...
For each 2048-second chunk of contiguous data, at each detector, the
...
auxiliary channels and \texttt{DARM\_ERR}.  These can be used to suggest
...
Livingston detector, characterized by a sudden drop in \texttt{DARM\_ERR}
...
A sample spike glitches in \texttt{DARM\_ERR}.  Note the characteristic
...
\texttt{DARM\_ERR}.  On the bottom left the time-frequency plot of a HEPI
...
looked for correlations between triggers from \texttt{DARM\_ERR} and auxiliary
...
\texttt{DARM\_ERR}.  Intervals where this percentage exceeded 50\% were entered
into the segment database and applied as category 4 vetoes.  Although
UPV was originally developed using Klinewelle triggers for both \texttt{DARM\_ERR} and
...
whose Klinewelle triggers correlate with daily ihope \texttt{DARM\_ERR} triggers.}
\end{figure}%
