...
compact binaries composed of neutron stars and/or black holes.  In
...
total mass of $25 \msun$.  Such waveforms may be trusted so long as
the velocities of the component masses are small compared to the speed
of light, or equivalently when the orbital frequencies are small.
Frequently the cutoff is taken to be the Schwarzschild ISCO frequency,
after which the inspiral transitions to a plunge, although this is
only strictly correct in the point-mass limit.
...
such systems and where possible optimize their use.  In order to
...
as described in section~\ref{sec:HybridWaveforms}.  This chapter uses
...
introduced in the following chapter.  Here we focus on a subset of
this question, the ability of pN waveforms to capture full physical
signals.  The tool for doing this will be the overlap
(equation~\ref{eq:OverlapDefinition}).  Clearly if no instance of a
waveform in the chosen pN model provides a high overlap with the
hybrid signal then the pipeline as a whole will be unable to detect
such signals.  A similar study has been performed by Pan~\etal using
...
extension of the TaylorF2 waveforms currently in use by the CBC
low-mass search yields high overlaps with numerical waveforms.
...
Sec.~\ref{sec:HybridWaveform} scaled to a range of masses.  We
