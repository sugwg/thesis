...
In chapter~ref{ch:pipeline} we outlined the CBC pipeline which is used
to search LIGO/Virgo data for gravitational waves resulting from 
the inspiral of compact binaries composed of neutron stars and/or
black holes.  In particular we noted the use of post-Newtonian
templates of the form~\ref{eqn:pn} to filter the data.  Such waveforms
may be trusted so long as the velocities of the component masses are
small compared to the speed of light, or equivalently when the orbital
frequencies are small.  A rule of thumb frequently used is that these
waveforms may be trusted up the Schwarzschild ISCO frequency, after
which the inspiral transitions to a plunge.

For systems containing two neutron stars this frequency is above 1000
Hz, even for a hypothetical system composed of two copies of the
...
therefore trust these waveforms for a BNS search.  However, for
systems including at least one black hole the ISCO frequency will be
lower, and the merger may even occur in the sensitive band.  It is
therefore important to test the ability of these templates to detect
such systems and where possible optimize their use.

In order to evaluate the effectiveness of templates it is necessary to
compare them against complete waveforms including the inspiral, merger
and ringdown phases.  This necessitates the use of hybrid pN/NR
waveforms as described in section~\ref{s:hybrids}.  This chapter uses
...
The most robust test would consider the ability of the pipeline as a
whole to detect BBH signals; this is the goal of the NINJA project
introduced in the following chapter.  Here we focus on a simpler
issue, the overlap between post-Newtonian and hybrid waveforms and
hence the ability of these waveforms to capture full physical signals.
A similar study has been performed by Pan~\etal using numerical data
from Pretorius and the Goddard groups~\cite{Pan2007}.  Our main
results are in agreement with their conclusion that a simple extension
of the existing stationary-phase approximation to the adiabatic
post-Newtonian waveforms (called \textit{TaylorF2} in
...
physical scale, and thus use standard mks units.  We use the overlap
defined in~\ref{eqn:overlap}.  To weight the inner product we use the
