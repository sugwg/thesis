...
the corresponding frequency is $f_{\textrm{ISCO}}=1/(6\sqrt{6}\pi M)$.
...
at $1.97\pm 0.04 \msun$)~\cite{Demorest:2010bx}).  This is well
...
waveform $\Psi_{4}^{2,2}$ (as defined, e.g., in Ref.~\cite{Boyle2008a}).
...
judge the \textit{TaylorF2} waveforms used in LIGO data analysis (see
Appendix A).  To understand how precisely we should trust our final
results, we need to understand the accuracy of the waveform itself.
The most obvious measure of the error in this fiducial waveform is its
convergence with increasing numerical resolution.
Fig.~\ref{f:accuracy} shows the overlap
(Eq.~\eqref{eq:OverlapDefinition}) between waveforms computed at
different resolutions.  The data used here are the extrapolated
...
also plan to study the impact of this change on the rate of spurious
triggers from glitches.
