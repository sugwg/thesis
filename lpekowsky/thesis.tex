...
effort is currently underway to make direct detections.  This network
of detectors includes the Interferometric Gravitational wave
Observatory (LIGO), which has recently completed its sixth science
run.

A particularly promising source of gravitational waves is astronomical
systems consisting of two neutron stars and/or black holes.  As the
objects orbit each other they emit gravitational radiation, lose
energy, and spiral inwards, producing a characteristic signal for
which we can search in the LIGO data.  Currently this is done using
matched-filter techniques, which matches the detector data against
analytic models.  Several choices must be made in constructing this
search.

In recent years numerical relativity has had impressive success in
simulating the final phases of the coalescence of binary black holes.
This has made it possible to perform realistic tests of the LIGO
searches.  Such tests can study both the degree to which analytic
models match the full waveforms and the ability of searches to make
detections.  The results of such tests can be used to improve the
...
database, and Peter Couvaras for all his help with condor and friendly
...
There are no factors of $2\pi$.  This convention differs from that
used in some gravitational wave literature, but is the adopted
convention in the LIGO Scientific Collaboration.
