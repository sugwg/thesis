...
curve. \Note{Should I include the definition here (and in other
chapters that use it) to remind the reader, and refer back to this
section for more details?}.  The waveforms were grouped into sets with
identical parameters.  For each set one waveform was chosen as the
reference and the overlap with all the other waveforms calculated at a
range of masses, optimizing over the unknown coalescence time and
phase as usual.  This process was then repeated taking each of the
other waveforms as the reference.
...
and is close to 1 at the lowest masses, where pN dominates.  That the
overlap diminishes slightly at intermediate masses must be due to the
different hybridization frequencies \Note{get these values}, and
indicates the sensitivity of the waveform to the hybridization details
(cite Ilana).
...
approximents and 0.94 for waveforms using the same approximent.  After
...
identical approximents.  The residual differences  between waveforms
...
BAM plus two different approximents as the template are shown in 
...
is enough of the merger in-band here to reduce the overlaps.  However,
...
value can stray further from the injected value.  As the hybridization
passes out of band this adjustment is no longer needed.


\section{Construction of the NINJA-2 data set}

In broad terms the plans for the NINJA-2 data sets followed those for
...
NINJA-1 consisted of only 127 injections in a day of data, this
...
is that data analysts can tune and test their pipelines on the dense
weeks, and then optional perform a self-blinded test on the final two
weeks.

In NINJA-1 the SNR was not chosen a priori but was determined by the
other parameters.  For NINJA-2 we draw the network SNR from a
distribution and then scale the distance of the injection in order to
achieve that SNR.  For the first three weeks the distribution is
linear from 6 to 130 in order to allow pipelines to test and tune out
to large SNRs on the densest set of injections.  For the remaining
weeks the distribution fell as the reciprocal of the SNR (uniform in
$\log(\mathrm{SNR})$) in order to better model the expected
astrophysical distribution.

The mass and waveform selection were also done slightly different in
...
selected masses, spins, and distances is shown in
...
As in NINJA-1 the sky localization and inclination were chosen
uniformly at random.
...
``burst/ringdown'' from $80-350 \msun$.  The original intend had been
...
pipelines to do sanity checks in the region to which they are most
sensitive.  These tests exposed several bugs in the injection code
which were fixed before generating the two-month data set.
...
from the previous (S5/VSR1) science runs as noise.  A key feature of
...
special-purpose tuning runs or analyses, these may lead to
short-author papers subject to the conditions of the NINJA agreement.
