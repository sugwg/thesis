...
quantitative studies of the behaviors of pipelines in varying regions
of signal parameter space.  Specifically, NINJA-2 addresses issues
with both the waveform submissions and the noise used to construct the
data sets.
...
Recall from section~\ref{sec:ihope_matched_filter} that the output of
...
figure~\ref{f:overlap_wiggles} which shows that the overlaps as a
...
respect to the sampling, and the overlap as a function of mass traces
...
waveforms submissions is shown in figure~\ref{ninja2_overlap_test}.
...
region is passing through the sensitive band.  The goal of
...
frequencies at which the hybridization is performed \Note{get these
values}.  The question of how many NR cycles are needed in order to
produce a robust hybrid waveform is an area of active
research~\cite{MacDonald:2011}. 
...
The same general bahvior can be seen in comparisons between 
non-spinning, unequal-mass ($q=2$) waveforms, shown in
figure~\ref{f:max_over_m_q2}.  However, the overlaps drop slightly at
...
\begin{figure}
  \includegraphics[width=0.5\linewidth]{figures/ninja2/maya_bam_q2_max_over_m} 
...
\begin{figure}
  \includegraphics[width=0.5\linewidth]{figures/ninja2/maya_llama_z0d4_max_over_m}
