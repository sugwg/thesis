...
\begin{equation}
$h_\mathrm{rms}$ = \sqrt{f |\tilde{s}(f)|^2},
\end{equation}
where $s$ is the detector strain output in the absence of a gravitational wave
signal. If the value of $h_c > h_\mathrm{rms}$, then the binary will be
...
significantly above the noise, so these MACHO binaries could be excellent source
for initial LIGO. Nakamura~\emph{et~al.}\cite{Nakamura:1997sm} showed that the
rate of MACHO binaries could be as high as $3$~yr$^{-1}$ at a distance of
$15$~Mpc\cite, under their model assuptions.
