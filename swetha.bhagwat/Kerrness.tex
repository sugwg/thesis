...
\newcommand{\alg}{%
\begin{figure*}[!htbp]
%\subfloat{\includegraphics[width=0.5\textwidth]{AlignmentPlot_orig.pdf} } 
\subfloat{\includegraphics[width=0.6\textwidth]{figures/AlignmentPlot_refreereport_2.pdf} }
%\subfloat{\includegraphics[width=0.45\textwidth, height=5.5cm]{AbsGlobalPeaksAlignedPlot.pdf} }
...
\begin{figure}[!htbp]
\centering
\subfloat{\includegraphics[width=0.7\columnwidth]{figures/Cartoon_Kerrness1.pdf} }
...
%\masha{SB + MO: Duncan wants to remove this header, but we think it's nice to single it out for people to refer to when reading the paper}
...
%We set $G = c = 1$ and express all quantities in code units%\note{MS: please remove the phrase 'code units'. In a paper (unless the paper is explicitly about details of the code), the units used in a particular code should be unimportant.}, given in terms of total
...
%%\note{ST: not sure what ``data development'' means here. Is there a clearer word we can use?} \swetha{SB + MO: Here, when we say data development we mean that $(Y,Y_{j})$ forms in both the spatial and temporal (and thus, in evolution) neighborhood of the point. Alfonso's paper calls this as data-development and hence we have used it here. Maybe we could put a footnote to define data-development.}   
...
%Here we combine all the quantities with equal weights, thereby presenting a conservative result. If one computes the sensitivity of the effective BH curvature potential to our Kerrness measures, one could weigh each of our measures proportional to the sensitivity. This would reduce the spread in each band in Fig.~\ref{fig:PercentOnTheWave}. However, investigating this is beyond the scope of this paper. 
...
%  %\note{MS: RWZ doesn't do extrapolation; extrapolation is a separate step at the end. Class. Quantum Grav. 26, 075009 (2009) describes RWZ implementation (emphasizing bcs in that paper rather than wave extraction).} which performs an extrapolation of the strain from a numerical simulation in powers of the extraction radius to obtain the strain at large radius.
%% Now with testing GR
...
%  then they may or may not continue to the rest of the paper.}
