\documentclass[aps,prd,showpacs,amssymb,floatfix,nofootinbib,superscriptaddress]{revtex4-1}%\usepackage{setspace}
...
\newcommand{\hGR}{\boldsymbol{h}_\mathrm{GR}}
\newcommand{\hAP}{\boldsymbol{h}_\mathrm{AP}}
\newcommand{\tbf}{\theta}
...
\newcommand{\WVU}{\affiliation{Department of Physics, West Virginia University, White Hall, Morgantown, WV 26506, USA}}
...
\title{Self-forced evolutions of an implicit rotating source: a natural framework to model  comparable and intermediate mass--ratio systems from inspiral through ringdown}
\author{E. A. Huerta}\email{elhuertaescudero@mail.wvu.edu}\WVU%
\author{Prayush Kumar}\email{pkumar@syr.edu}\SU%
\author{Jonathan R. Gair}\email{jgair@ast.cam.ac.uk}\CU%
\author{Sean T. McWilliams}\email{sean.mcwilliams@mail.wvu.edu}\WVU%

\date{\today}

\begin{abstract} 
We develop a waveform model to describe the inspiral, merger and ringdown of binary systems with comparable and intermediate mass-ratios. This model incorporates first-order conservative self-force corrections to the energy and angular momentum, which are valid in the strong--field regime~\cite{Akcay:2012}. We model the radiative part of the self-force by deriving second-order radiative corrections to the energy flux. These corrections are obtained by minimizing the phase discrepancy between our self-force model and the effective one body model~\cite{buho, Damour:2013} for a variety of mass-ratios. We show that our model performs substantially better than post-Newtonian approximants currently used to model neutron star-black hole mergers from early inspiral to the innermost stable circular orbit. In order to match the late inspiral evolution onto the plunge regime, we extend the ``transition phase'' developed by Ori and Thorne~\cite{amos} by including finite mass--ratio corrections and modelling the orbital phase evolution using an implicit rotating source~\cite{Baker:2008}. We explicitly show that the implicit rotating source approach provides a natural transition from late-time radiation to ringdown that is equivalent to ringdown waveform modelling based on a sum of quasinormal modes. 
...
\begin{figure*}[ht]
\centerline{
\includegraphics[height=0.35\textwidth,  clip]{m1m006_phasedifftM_T4EOB_r030tM.eps}
\includegraphics[height=0.35\textwidth,  clip]{m1m008_phasedifftM_T4EOB_r030_tM.eps}
}
\centerline{
\includegraphics[height=0.35\textwidth,  clip]{m1m010_phasedifftM_T4EOB_r030tM.eps}
\includegraphics[height=0.35\textwidth,  clip]{m1m015_phasedifftM_T4EOB_r030_tM.eps}
}
...
\begin{figure*}[ht]
\centerline{
\includegraphics[height=0.35\textwidth,  clip]{b1b2_mapb1b2m1m6no.eps}
\includegraphics[height=0.35\textwidth,  clip]{b3b4_mapb3b4m1m6no.eps}
}
\centerline{
\includegraphics[height=0.35\textwidth,  clip]{b1b2_mapb1b2m1m8no.eps}
\includegraphics[height=0.35\textwidth,  clip]{b3b4_mapb3b4m1m8no.eps}
}
...
It must be emphasized that even if we only use the inspiral evolution to model binaries with mass-ratios that typically describe NSBH binaries, our self-force evolution model performs better than TaylorT4, since we can reduce the phase discrepancy between TaylorT4 and EOB at the last stable circular orbit by a factor of \((\sim40, \, \sim70)\)  for binaries with \(q=(1/6,\,1/10)\)  and total mass \(M_{{\rm{tot}}}\in (7M_{\odot} ,\, 11M_{\odot} )\) (see Figure~\ref{pn_approx}).
 ...
 \begin{figure*}[ht]
\centerline{
\includegraphics[height=0.35\textwidth,  clip]{phsiffeobsfm1m6.eps}
\includegraphics[height=0.35\textwidth,  clip]{phsiffeobsfm1m8.eps}
}
\centerline{
\includegraphics[height=0.35\textwidth,  clip]{phsiffeobsfm1m10.eps}
\includegraphics[height=0.35\textwidth,  clip]{phsiffeobsfm1m15.eps}
}
...
\begin{figure*}[ht]
\centerline{
\includegraphics[height=0.32\textwidth,  clip]{effective_potential_nonpert.eps}
\includegraphics[height=0.32\textwidth,  clip]{effective_potential_pert.eps}
}
...
\begin{figure*}[ht]
\centerline{
\includegraphics[height=0.36\textwidth,  clip]{rvstm1m6.eps}
\includegraphics[height=0.36\textwidth,  clip]{xvsMOmegam1m6.eps}
}
\centerline{
\includegraphics[height=0.36\textwidth,  clip]{rvstm1m8.eps}
\includegraphics[height=0.36\textwidth,  clip]{xvsMOmegam1m8.eps}
}
...
\begin{figure*}[ht]
\centerline{
\includegraphics[height=0.36\textwidth,  clip]{rvstm1m10.eps}
\includegraphics[height=0.36\textwidth,  clip]{xvsMOmegam1m10.eps}
}
\centerline{
\includegraphics[height=0.36\textwidth,  clip]{rvstm1m15.eps}
\includegraphics[height=0.36\textwidth,  clip]{xvsMOmegam1m15.eps}
}
...
\begin{figure*}[ht]
\centerline{
\includegraphics[height=0.35\textwidth,  clip]{wavem1m6complete.eps}
\includegraphics[height=0.35\textwidth,  clip]{wavem1m8complete.eps}
}
\centerline{
\includegraphics[height=0.35\textwidth,  clip]{wavem1m10complete.eps}
\includegraphics[height=0.35\textwidth,  clip]{wavem1m15complete.eps}
}
