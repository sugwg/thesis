...
is unimportant. See for example \cite{Thorne:1982cv} for a full treatment of
...
interferometer \emph{locked}. The optics and control loop form the core
systems of the interferometer, however there are many subtleties to the design
and operation of a gravitataional wave interferometer this which are outside
...
feedback filter, $D(f)$\cite{Gonzalez:2002}. 
...
m_2$, then Newtonian gravity will be provide a good description of the binary
as the velocities of the holes will be much less than the speed of light. We
may therefore use the \emph{quadrupole formula} to calculate the gravitational
radiation emitted by the binary\cite{MTW:1973}
\begin{equation}
h_{jk}^\mathrm{TT} = 
...
\label{eq:massquad}
\end{equation}
and $\mathcal{I}_{jk}^\mathrm{TT}$ is the transverce traceless part of of
...
\end{equation}
Let us position the origin of a cartestian coordinate system, $(x,y,z)$, at
...
\end{align}
Let us now introduce a second, spherical polar coordinate system labelled
$(r,\iota,\phi_c)$ related to the cartesian coordinate system by
\begin{align}
\boldsymbol{e}_{\hat{\iota}} &= 
\cos \iota \cos \phi_c \boldsymbol{e}_{\hat{x}} 
+ \cos \iota \sin \phi_c \boldsymbol{e}_{\hat{y}} 
- \sin \iota \boldsymbol{e}_{\hat{z}}, \\
\boldsymbol{e}_{\hat{\phi}_c} &= 
- \sin \phi_c \boldsymbol{e}_{\hat{x}} 
+ \cos \phi_c \boldsymbol{e}_{\hat{y}}.
\end{align}
Let us calculate the gravitational radiation $h_{jk}^\mathrm{TT}$ seen by an
observer at a position $(r,\iota,\phi_c)$ relative to the center of mass of
...
\ddot{\mathcal{I}}_{xx} \cos^2 \iota \cos^2 \phi_c 
+  \ddot{\mathcal{I}}_{yy} \cos^2 \iota \sin^2 \phi_c 
+ 2  \ddot{\mathcal{I}}_{xy} \cos^2\iota\sin\phi_c\cos\phi_c \\
&= - 2\mu a^2 \Omega^2 \cos(2\Omega t) \cos^2\iota \cos 2\phi_c
-2 \mu a^2 \Omega^2 \sin(2\Omega t) \cos^2\iota \sin 2\phi_c.
\end{split}
\end{equation}
Similar transformations give the other components of $\ddot{\mathcal{I}}_jk$
\begin{align}
\ddot{\mathcal{I}}_{\phi_c\phi_c} &=
2\mu a^2 \Omega^2 \cos(2\Omega t) \cos 2\phi_c
+ \frac{1}{2} \mu a^2 \Omega^2 \sin(2\Omega t) \sin 2\phi_c, \\
\ddot{\mathcal{I}}_{\iota\phi_c} = 
\ddot{\mathcal{I}}_{\phi_c\iota} &= 
- 2 \mu a^2 \Omega^2 \cos\theta \sin\left(2\Omega t - 2\phi_c\right).
\end{align}
...
\begin{equation}
\begin{split}
\ddot{\mathcal{I}}_{\iota\iota}^\mathrm{TT} &= 
\ddot{\mathcal{I}}_{\iota\iota} - \frac{1}{2}
\left( \ddot{\mathcal{I}}_{\iota\iota} + 
\ddot{\mathcal{I}}_{\phi_c\phi_c} \right) = 
\frac{1}{2} \left(  \ddot{\mathcal{I}}_{\iota\iota} - 
\ddot{\mathcal{I}}_{\phi_c\phi_c}
\right) \\
&= \mu a^2\Omega^2 \left(1 + \cos^2\iota\right) \cos\left(2\Omega t - 2\phi_c\right).
\end{split}
\end{equation}
...
-\frac{2\mu a^2\Omega^2}{r} (1 + \cos^2\iota) \cos\left(2\Omega t - 2\phi_c\right) \\
h_{\iota\phi_c}^\mathrm{TT} &= 
\frac{2}{r} \ddot{\mathcal{I}}_{\iota\iota}^\mathrm{TT} =
-\frac{4 \mu a^2 \Omega^2}{r} \cos\theta \sin\left(2\Omega t - 2\phi_c\right).
\end{align}
...
f  = \frac{2\Omega}{2\pi}.
\end{equation}
and if we use the basis vectors $\boldsymbol{e}_{\iota}$ and
$\boldsymbol{e}_{\phi_c}$ as the polarization axes of the gravitational wave
we obtain
\begin{align}
h_{+}(t) &= h_{\iota\iota}^\mathrm{TT} = - \frac{2G}{c^2 r} \mu (\pi M
f)^\frac{2}{3} (1 + \cos^2 \iota ) \cos(2\pi f t - 2 \phi_c) \\
h_{\times}(t) &= h_{\iota\phi_c}^\mathrm{TT} = - \frac{4G}{c^2 r} \mu (\pi M
f)^\frac{2}{3} \cos\iota \sin(2\pi f t - 2 \phi_c)
\end{align}
The angles $\iota$ and $\phi_c$ ar known as the \emph{inclination angle} and
the \emph{coalescence phase}, respectively and $r$ is the distance from the
observer to the binary.  

Finally, let us define the \emph{cosine chirp}, $h_c$,  and the \emph{sine
chirp}, $h_s$, as 
\begin{align}
h_c(t) & = 2\mu (\pi M f)^{\frac{2}{3}} \cos(2\pi f t - 2\phi_c), \\
h_s(t) & = 2\mu (\pi M f)^{\frac{2}{3}} \sin(2\pi f t - 2\phi_c),
\end{align}
so the plus and cross waveforms can be written as
\begin{align}
...
in section \ref{s:inspiralgw}. There are two ways of obtaining $\tilde{h_c}(f)$.
