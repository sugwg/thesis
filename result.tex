...
\end{center}
\caption[Injections Found and Missed by Software Monte Carlo]{%
\label{f:m1m2_found_missed}
This figure shows the results of the Monte Carlo simulation used to measure
the efficiency of the pipeline once parameter tuning had been performed.  The
injections that were detected are shown as circles on the upper plot and those
that were not found are shown by triangles on the lower plot. The $x$ and $y$
coordinates of each injection are the mass parameters $m_1$ and $m_2$,
respectvely, and the color of each injection represents the effective distance
at which the waveform was injected. 
