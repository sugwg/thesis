...
a re-weighted SNR of 8 in the time scales used for the O1 analysis. 
(For more discussion on the output of the PyCBC search 
...
noisy frequencies of the signal coherent with other signals in the instrument 
...
$h(t)$ and the auxiliary channel based solely on chance, $\mu$, is estimated as \cite{Smith:2011},
\begin{equation}
...
\subsection{Analog-to-Digital Conversion Overflows}\label{sec:adc-overflow}
...
\caption[Example of a feedback loop]{A simplified visualization of a feedback %
         ...
         the alignment of the optic. The analog signal is sent through an analog actuator %
         that physically interects with the optic to correct its motion.}
...
The effects of this issue were visible in the $h(t)$ channel in engineering 
...
\caption[Vetoed DARM triggers from DAC calibration]{A time-frequency %
         ...
         The blue dots indicate glitches in the DARM degree of freedom, %
         ...
         auxiliary channel triggers in this case indicate that the control signal %
...
\begin{table}[ht!]%
 ...
 \begin{center}
  ...
  \hline
  \begin{tabular}{@{}c@{}}ETMY control signal \\ crosses $2^{16}$ \end{tabular} & 
...
\caption[Median noise ASD in O1]{Median noise amplitude spectral density (ASD) %
         ...
         and mechanical resonances. This plot is constructed by taking the %
         Fourier transform of $h(t)$ and averaging several measurements of the %
         power spectral density (PSD). The average PSD is then square rooted %
         to calculate the ASD. 
         }
...
\caption[Inspiral horizon distance in O1]{Inspiral horizon distance for sources with %
         ...
         which a signal could be recovered at SNR 8 if the binary system %
         was at the optimal sky location, which is directly above the detector, % 
         and optimal orientation, which means that the orbital plane of the binary system %
         was parallel with the plane of the detector. The inspiral horizon distance for %
